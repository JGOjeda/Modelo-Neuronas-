...
contador= contador+1;
 ...
 for iii = 1: length(HVIP)
     ...
     HSST1= HSST(iii); 
     HSST1= cell2mat(HSST1);
     HSST2{iii}= HSST1';

     HVIP1= HVIP(iii); 
     HVIP1= cell2mat(HVIP1);
     HVIP2{iii}= HVIP1';
...
% HE2= cell2mat(HE2);
% HFS2= cell2mat(HFS2);
% HSST2= cell2mat(HSST2);
% HVIP2= cell2mat(HVIP2);
...
% 
 Ra2(i)= max(Ra); %copiar y pegar en sigma o exel para su promedio 
...
%      S2 = randi([0 70],1,1); %apertura
    ...
    alpha_m(i) = Y*(0.1*(VE(i)+40)/(1-exp(-(VE(i)+40)/10)));
    beta_m(i) =Y*(4*exp(-(VE(i)+65)/20)); %Equation 21   Na

    alpha_h(i) =Y*(0.07*exp(-(VE(i)+65)/20)); %Equation 23   Na
